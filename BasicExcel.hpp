...
// Version 1.15 (18 July 2021)
// From this version, modifications are made by Spring (C. Huang).
    // - Add Block::OpenChar() so that it will accept filename of char* type.
    // - Add CompoundFile::OpenChar() so that it will accept filename of char* type.
// Version 1.16 (19 July 2021)
    // - Add CompoundFile::CreateChar(const char* filename) so that it will accept filename of char* type.
    // - Add Block::CreateChar(const char* filename) so that it will accept filename of char* type.
    // - Modify BasicExcel::SaveAs(const char* filename) so that it will accept filename of char* type.
...
// File handling functions
    ...
    bool CreateChar(const char* filename);
	bool Open(const wchar_t* filename, ios_base::openmode mode=ios_base::in | ios_base::out);
    bool OpenChar(const char* filename, ios_base::openmode mode=ios_base::in | ios_base::out);
...
struct LittleEndian
{
    ...
    // ATTENTION: memory for name is applied in this function, it should be freed afterwards.
    // PROMISE: Return true if wname is successfully transferred, false if otherwise.
    {
